...
Pi-hole is now successfully installed. It is now necessary for you to point your devices to this DNS server. The first way of doing this is by configuring each device  to use your newly created Pi-hole DNS server. The second option is to configure your router to use your Pi-hole DNS server by default, which is my preferred method. Configuring the system this way forces all devices to automatically use this DNS server, which means any device connected to your router will automatically be using Pi-hole.Open your router's domain page and go to DNS address.(I am not showing exactly the steps because the process change from router to router,just Google to find more informtion about your own system)
